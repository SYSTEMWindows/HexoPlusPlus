window.document.oncontextmenu = function(event) {
  ...
  function Mirror() {
    ...
    if (hostname === 'www.MrZeFr.ml') {
      url = "https://www.MrZeFr.tk" + pathname;
      window.alert("即将前往MrZeFr的引导站🍬");
      window.location.href = url;
    }
    else if(hostname === 'www.MrZeFr.tk') {
      url = "https://www.MrZeFr.ml" + pathname;
      window.alert("正在返回雪中的博客🍭！");
      window.location.href = url;
    }
    else {
      window.alert("本地调试不需要跳转哦！🍫");
    }
  }
  //评论窗口跳转
  function ToComment(){
